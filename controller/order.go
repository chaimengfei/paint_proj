...
func (oc *OrderController) PayOrder(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&req); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"code": -1, "message": "订单payment_type格式错误"})
		return
	}
	paymentDataMap, err := oc.orderService.PayOrder(c.Request.Context(), userID, orderID, req.PaymentType)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"code": -1, "message": err.Error()})
		return
	}
	c.JSON(http.StatusOK, gin.H{"code": 0, "data": paymentDataMap})
}
...
func (oc *OrderController) PaymentCallback(c *gin.Context) {
	...
	if err := oc.orderService.OrderPaidCallback(c.Request.Context(), req); err != nil {
